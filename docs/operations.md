...
待ち時間はプロンプトの読み込み（プレフィル）が大半を占めるため、**図種によって数倍違います**。
まず動作を確かめたいときは軽い図種から試すのが早道です。

| 目安 | 図種（プロンプト長） |
|---|---|
| 軽い | ユーザージャーニー図（1.1K）／ネットワークパケット図（1.6K）／円グラフ（1.8K）／アーキテクチャ図（2.2K） |
| 中くらい | 要件図（3.2K）／サンキーチャート（4.2K）／マインドマップ（4.5K）／XYチャート（4.6K）／タイムライン図（4.9K）／4象限チャート（5.9K） |
| 重い | 状態遷移図（8.0K）／Gitグラフ（8.7K）／ER図（9.0K）／シーケンス図（9.1K）／ブロック図（10.2K）／クラス図（10.3K）／ガントチャート（11.1K）／フローチャート（15.9K） |

（文字数。上記「約 20 分」は最も重いフローチャートの実測値です。）

#### うまく描画されないとき

- **同じ依頼でも出力は毎回変わります。** 失敗したら再実行を試してください。ローカルの 8B 級
  モデルは記法を外すことがあり、成否が揺れます。
- **モデルセレクタが 8B 級になっているか確認してください。** 3B 級では記法が崩れて描画できません。
- 図が出ずエラー文が表示された場合、原因はほぼモデルの出力側です。結果画面の
  「コードを表示」タブで、生成された mermaid 記法を確認できます。

なお **web 側のプロンプトと mermaid の記法補正は、このローカル小型モデル前提に合わせて
調整済み**です（日本語ラベルの引用符補完など、モデルが外しやすい部分は自動で補正します）。
利用者側の設定は不要です。調整の一覧と理由は `genai-web-onpre` の `UPSTREAM.md` §6 にあります。

2026-09-02 の実機検証（ELYZA-8B・CPU 11GB 機）では、アーキテクチャ図／XYチャート／円グラフ／
マインドマップ／Gitグラフ／シーケンス図の描画を確認しています。
